 ...
 * 
 * This release of Arca SDK is intended to facilitate 
 * the integration to other different web services that 
 * Electronic Billing   
 *
 * @link http://www.afip.gob.ar/ws/ ARCA Web Services documentation
 *
 * @package Arca
 **/
...
class Arca {
	...
	function __construct($options)
	{
		...
		if (!isset($options['res_folder'])) {
			$this->RES_FOLDER = __DIR__.'/resources/';
		} else {
			$this->RES_FOLDER = $options['res_folder'];
		}

		if (!isset($options['ta_folder'])) {
			$this->TA_FOLDER = __DIR__.'/resources/';
		} else {
	...
	/**
	 * Gets token authorization for an ARCA Web Service
	 *
	 * @since 0.1
	 *
	 ...
	 *
	 * @return TokenAuthorization Token Authorization for ARCA Web Service 
	**/
	...
	**/
	 ...
	 * @throws Exception if an error occurs
	 *
	 * @return ArcaWebService Token Authorization for ARCA Web Service 
	 **/
	public function WebService($service, $options)
	{
		...
		return new ArcaWebService($this, $options);
	}
...
/**
 * Base class for ARCA web services 
 *
 * @since 0.6
 *
 * @package Arca
...
class ArcaWebService
{
	...
	function __construct($arca, $options = array())
	{
		$this->arca = $arca;
		$this->options = $options;
		...
		if (isset($options['generic']) && $options['generic'] === TRUE) {
			...
			if ($this->arca->options['production'] === TRUE) {
			...
			if ($this->arca->options['production'] === TRUE) {
				$this->WSDL = __DIR__.'/resources/'.$this->WSDL;
			} else {
				$this->WSDL = __DIR__.'/resources/'.$this->WSDL_TEST;
	...
	{
		return $this->arca->GetServiceTA($this->options['service']);
	}
	...
	public function ExecuteRequest($operation, $params = array())
	{
		if (!isset($this->soap_client)) {
			$this->soap_client = new SoapClient($this->WSDL, array(
				...
				'location'       => $this->URL,
				'trace'          => 1,
				'exceptions'     => $this->arca->options['exceptions'],
				'stream_context' => stream_context_create(['ssl'=> ['ciphers'=> 'AES256-SHA','verify_peer'=> false,'verify_peer_name'=> false]])
			));
		}
	...
	/**
	 * It normalizes responses, avoids receiving objects, and 
	 * converts them into multidimensional arrays.
	 * 
	 * @since 0.6
	 *
	 * @param string 	$data 	ARCA response 
	 * 
	 * @return mixed 
	 **/
	protected function normalizeResponse($data) {
		// Si es un objeto, lo convertimos a array recursivamente
		if (is_object($data)) {
			$data = json_decode(json_encode($data), true);
		}
	
		// Si ya es un array, nos aseguramos de que sus hijos también sean arrays
		// (por si es un array que contiene objetos stdClass adentro)
		if (is_array($data)) {
			foreach ($data as $key => $value) {
				$data[$key] = $this->normalizeResponse($value);
			}
		}
